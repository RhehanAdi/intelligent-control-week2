...
svm = SVC(kernel='rbf', probability=True)  # bisa pakai 'linear' juga
svm.fit(X_scaled, y)
...
while True:
    ...
    # titik pengukuran (kiri, tengah, kanan)
    points = [
        (w // 4, cy),
        (cx, cy),
        (3 * w // 4, cy),
    ...
    for px, py in points:
        ...
        # Prediksi dengan SVM
        probs = svm.predict_proba(pixel_scaled)[0]
        color_pred = svm.classes_[np.argmax(probs)]
        accuracy = np.max(probs) * 100

        results.append((color_pred, accuracy, (b, g, r), (x1, y1, x2, y2), (px, py)))

    # gambar kotak pengukuran
